...
function Recommendation() {
            ...
            try {
                ...
                if (activeCategory === "All") {
                    ...
                    const categoryPromises = categories.slice(1).map(async (category) => {
                        const response = await fetch(`https://gemini-refresh.duckdns.org/api/suggest-cuisines?category=${category}`);
                    ...
                    const response = await fetch(`https://gemini-refresh.duckdns.org/api/suggest-cuisines?category=${activeCategory}`);
    ...
    const recommendationItems = Array.isArray(recommendation) 
        ? recommendation.map(item => ({
            ...item,
            src: `/assets/${item.category?.toLowerCase() || 'default'}.jpg` || '/assets/recommended-default.jpg',
        ...
        <RecipeDetailsProvider>
            <div className="overflow-x-hidden relative">
                ...
                <div className="relative flex px-6 pt-8 pb-20">
                    <p className="text-4xl font-bold absolute top-10">Best Meals <br /> for you</p>
                    <Image src="/assets/burger.png" alt="burger" width={350} height={200} className="absolute -right-2 top-0"/>
                </div>
